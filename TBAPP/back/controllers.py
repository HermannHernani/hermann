...
@app.route("/api/verifyPasswordToken", methods=['POST'])
...
@app.route("/api/updatePasswordViaEmail",methods=['PUT'])
@require_appkey
...
@app.route('/api/register/tratamento',methods=['POST'])
@token_required
def register_tratamento(current_user):
    ...
    response.headers['Access-Control-Allow-Origin'] = '*'
    return response
@app.route('/api/tratamento/<string:dia>',methods=['GET'])
@token_required
@require_appkey
...
@app.route('/api/datas/<string:dates>', methods=['GET'])
@require_appkey
@token_required
...
#PEGAR FOTO SALVO
@app.route('/api/imgs/uploads/<string:path>', methods=['GET'])
